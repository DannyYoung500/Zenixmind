...
function PublicHome() {
  return <main className="min-h-screen bg-[#050506] text-zinc-100">
    ...
    <section className="relative z-10 mx-auto flex min-h-[calc(100vh-72px)] max-w-6xl flex-col items-center justify-center px-5 py-20 text-center sm:px-8">
      ...
      <div className="mt-10 w-full max-w-3xl"><Link href="/assistant" className="group block rounded-[28px] border border-white/[.1] bg-[#0b0b0d] p-4 text-left shadow-[0_30px_100px_rgba(0,0,0,.35)] hover:border-white/[.18]"><div className="min-h-[74px] px-3 py-2 text-sm text-zinc-600">Ask ZenixMind anything…</div><div className="flex items-center justify-between"><div className="flex items-center gap-1"><span className="grid h-10 w-10 place-items-center rounded-xl text-zinc-500"><Icon name="plus"/></span><span className="grid h-10 w-10 place-items-center rounded-xl text-zinc-500"><Icon name="mic"/></span></div><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#171719] text-zinc-400 group-hover:text-white"><Icon name="arrow"/></span></div></Link><p className="mt-3 text-[10px] text-zinc-700">Start a real conversation. Your chats are saved to your account.</p></div>
      <div className="mt-10 grid w-full max-w-3xl grid-cols-2 gap-2 sm:grid-cols-4">{[["Chat","Ask, reason and work through ideas."],["Voice","Talk naturally with ZenixMind."],["Images","Create visual work when image generation is connected."],["Library","Keep your uploaded and generated files together."]].map(([title,body])=><Link key={title} href={title==="Chat"?"/assistant":title==="Voice"?"/assistant/voice":title==="Images"?"/assistant?view=images":"/assistant?view=library"} className="rounded-2xl border border-white/[.06] bg-[#09090b] p-4 text-left hover:border-white/[.12]"><p className="text-xs font-medium text-zinc-200">{title}</p><p className="mt-2 text-[10px] leading-5 text-zinc-600">{body}</p></Link>)}</div>
    </section>
  </main>;
}
