      ...
      <div className="flex min-h-screen">
        <aside className="hidden w-[272px] shrink-0 flex-col bg-[#171717] px-3 py-3 lg:flex">
          <div className="flex items-center justify-between px-2">
            <button className="grid h-10 w-10 place-items-center rounded-full text-[#c4c7c5] hover:bg-black/50"><Icon name="menu"/></button>
            <BrandMark size={29}/>
          </div>
          ...
          <div className="mt-2 space-y-0.5">{recent.map((item, i) => <Link key={item} href="/assistant" className={`flex items-center rounded-full px-4 py-2.5 text-[13px] text-[#c4c7c5] hover:bg-black/50 ${i === 0 ? "bg-black/[.45]" : ""}`}>{item}</Link>)}</div>
          <div className="mt-auto space-y-1">
            <Link href="/dashboard" className="block rounded-full px-4 py-3 text-sm text-[#c4c7c5] hover:bg-black/50">Your chats</Link>
            <Link href="/pricing" className="block rounded-full px-4 py-3 text-sm text-[#c4c7c5] hover:bg-black/50">Upgrade</Link>
            <Link href="/login" className="block rounded-full px-4 py-3 text-sm text-[#c4c7c5] hover:bg-black/50">Account</Link>
          </div>
        </aside>
        <section className="flex min-w-0 flex-1 flex-col bg-[#0b0b0b]">
          <header className="flex h-16 items-center justify-between px-4 sm:px-7">
            <div className="flex items-center gap-3 lg:hidden"><button className="grid h-10 w-10 place-items-center rounded-full hover:bg-black/50"><Icon name="menu"/></button><BrandMark size={28}/></div>
            <div className="hidden items-center gap-2 lg:flex"><span className="text-[17px] font-medium">ZenixMind</span><span className="rounded-md bg-black/[.45] px-2 py-1 text-[11px] text-[#9aa0a6]">AI</span></div>
            <div className="flex items-center gap-1"><button className="grid h-10 w-10 place-items-center rounded-full text-[#9aa0a6] hover:bg-black/50"><Icon name="search"/></button><Link href="/login" className="grid h-10 min-w-10 place-items-center rounded-full bg-[#a8c7fa] px-4 text-sm font-medium text-[#111]">Sign in</Link></div>
          </header>
              ...
              <div className="mx-auto w-full max-w-[760px]">
                <div className="rounded-[28px] border border-[#3c4043] bg-[#202124] p-3 shadow-[0_2px_12px_rgba(0,0,0,.25)]">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex gap-1"><button className="grid h-10 w-10 place-items-center rounded-full text-[#bdc1c6] hover:bg-black/50"><Icon name="plus"/></button><button className="grid h-10 w-10 place-items-center rounded-full text-[#bdc1c6] hover:bg-black/50"><Icon name="file"/></button><button className="grid h-10 w-10 place-items-center rounded-full text-[#bdc1c6] hover:bg-black/50"><Icon name="mic"/></button></div>
                    <Link href="/assistant" className="grid h-10 w-10 place-items-center rounded-full bg-[#171717] text-zinc-200 border border-white/10 hover:bg-[#222]"><Icon name="send" size={18}/></Link>
                  </div>
                </div>
