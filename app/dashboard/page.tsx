    ...
    <main className="min-h-screen bg-[#060607] text-white">
      <div className="flex min-h-screen">
        <aside className="hidden w-[248px] shrink-0 border-r border-white/[.06] bg-[#09090b] p-3 lg:flex lg:flex-col">
          ...
          <Link href="/assistant" className="mt-5 flex items-center justify-center rounded-xl bg-[#171717] px-4 py-2.5 text-sm font-semibold text-black transition hover:bg-[#222]">+ New chat</Link>
          <div className="mt-7">
            <p className="px-2.5 text-[10px] font-medium uppercase tracking-[.18em] text-zinc-700">Recent chats</p>
            <div className="mt-2 space-y-0.5">
              {(conversations || []).map((chat) => (
                <Link key={chat.id} href={`/assistant?conversation=${chat.id}`} className="block truncate rounded-lg px-2.5 py-2.5 text-[12px] text-zinc-500 transition hover:bg-black/[.45] hover:text-zinc-200">{chat.title}</Link>
              ))}
          ...
          <div className="mt-auto space-y-1 border-t border-white/[.06] pt-3">
            <Link href="/assistant" className="block rounded-lg px-2.5 py-2.5 text-xs text-zinc-600 hover:bg-black/[.45] hover:text-white">Assistant</Link>
            <Link href="/owner" className="block rounded-lg px-2.5 py-2.5 text-xs text-zinc-600 hover:bg-black/[.45] hover:text-white">Owner console</Link>
          </div>
        </aside>
        ...
        <section className="min-w-0 flex-1">
          <header className="flex h-[68px] items-center justify-between border-b border-white/[.06] px-5 sm:px-8">
            ...
            <div className="flex items-center gap-2">
              <Link href="/assistant" className="rounded-lg border border-white/[.08] px-3 py-2 text-xs text-zinc-400 hover:bg-black/[.45] hover:text-white">Open assistant</Link>
              <div className="grid h-8 w-8 place-items-center rounded-full border border-white/[.08] bg-black/[.45] text-[11px] text-zinc-300">{firstName[0]?.toUpperCase()}</div>
            </div>
          </header>
          ...
          <div className="mx-auto max-w-5xl px-5 py-10 sm:px-8 sm:py-14">
            <div className="flex flex-col gap-8 md:flex-row md:items-end md:justify-between">
              ...
              <Link href="/assistant" className="inline-flex h-11 shrink-0 items-center justify-center gap-2 rounded-xl bg-[#171717] px-5 text-sm font-semibold text-black hover:bg-[#222]">New chat <Arrow /></Link>
            </div>

            <div className="mt-12 grid gap-3 md:grid-cols-3">
              <Link href="/assistant" className="group rounded-2xl border border-white/[.07] bg-black/[.45] p-5 transition hover:border-white/[.12] hover:bg-black/[.45]">
              ...
              <Link href="/pricing" className="group rounded-2xl border border-white/[.07] bg-black/[.45] p-5 transition hover:border-white/[.12] hover:bg-black/[.45]">
            ...
            <section className="mt-12">
              ...
              <div className="mt-4 overflow-hidden rounded-2xl border border-white/[.07] bg-black/[.45]">
                {(conversations || []).map((chat) => (
                  <Link key={chat.id} href={`/assistant?conversation=${chat.id}`} className="flex items-center justify-between gap-4 border-b border-white/[.05] px-5 py-4 last:border-0 hover:bg-black/[.45]">
