...
export default async function OwnerDashboard() {
  ...
  return <WorkspaceShell active="owner" title="Owner console">
    <div className="mx-auto max-w-6xl px-5 py-9 sm:px-8 sm:py-12">
      ...
      <section className="mt-10"><p className="text-xs font-medium uppercase tracking-[.16em] text-zinc-600">Platform</p><h2 className="mt-2 text-xl font-semibold">Management areas</h2><div className="mt-5 grid gap-3 md:grid-cols-2">{sections.map(([title,body,status])=><div key={title} className="rounded-2xl border border-white/8 bg-black/[.45] p-6"><div className="flex items-start justify-between gap-4"><h3 className="font-medium">{title}</h3><span className="rounded-full border border-white/8 px-2.5 py-1 text-[10px] text-zinc-500">{status}</span></div><p className="mt-3 text-sm leading-6 text-zinc-500">{body}</p></div>)}</div></section>
